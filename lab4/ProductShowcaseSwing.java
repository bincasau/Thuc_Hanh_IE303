...
public class ProductShowcaseSwing extends JFrame {
    ...
    public ProductShowcaseSwing(List<Product> products) {
        super("Adidas Product Showcase");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setMinimumSize(new Dimension(1200, 720));
        setPreferredSize(new Dimension(1280, 760));
        setContentPane(buildContent(products));
        pack();
    ...
    private JPanel buildContent(List<Product> products) {
    ...
    public static void showUi(List<Product> products) {
        SwingUtilities.invokeLater(() -> {
            try {
                UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
            } catch (Exception ignored) {
            }
            new ProductShowcaseSwing(products).setVisible(true);
        });
    }
    ...
    private static class ProductCard extends JPanel {
        ...
        private ProductCard(Product product) {
            ...
            JLabel nameLabel = new JLabel(truncate(product.getName(), 16));
            nameLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
            nameLabel.setFont(new Font("Segoe UI", Font.BOLD, 14));
            nameLabel.setForeground(new Color(68, 68, 68));

            JLabel subtitleLabel = new JLabel(truncate(product.getDescription(), 23));
            subtitleLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
            subtitleLabel.setFont(new Font("Segoe UI", Font.BOLD, 11));
            subtitleLabel.setForeground(new Color(190, 190, 190));

            JLabel imageLabel = new JLabel(new javax.swing.ImageIcon(scaleImage(loadImage(product.getImagePath()), 200, 200)));
            ...
            JLabel priceLabel = new JLabel(PRICE_FORMAT.format(product.getPrice()));
    ...
    private static class ProductDetailPanel extends JPanel {
        ...
        private void paintProduct(Graphics2D g2, Product product, float alpha, int xOffset) {
            ...
            Image scaledImage = scaleImage(loadImage(product.getImagePath()), width - 36, imageAreaHeight - 30);
            ...
            drawWrappedText(layer, product.getName(), 16, textY, width - 32, 26, 2);

            layer.setFont(new Font("Segoe UI", Font.BOLD, 20));
            layer.drawString(PRICE_FORMAT.format(product.getPrice()), 16, textY + 48);

            layer.setFont(new Font("Segoe UI", Font.PLAIN, 13));
            layer.setColor(new Color(87, 87, 87));
            layer.drawString(product.getBrand(), 16, textY + 72);

            layer.setFont(new Font("Segoe UI", Font.BOLD, 13));
            layer.setColor(new Color(165, 165, 165));
            drawWrappedText(layer, product.getDescription(), 16, textY + 98, width - 32, 20, 4);
            layer.dispose();
        }
    }
}
